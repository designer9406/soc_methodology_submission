...
void layer_forward_32_cpu(struct layer* layer, float* src, float* dest, float* workspace)
{
  switch (layer->type)
    {
    case CONVOLUTIONAL: { convolutional_precomp_forward_32_cpu(layer, src, dest, workspace); break; }
    case MAXPOOL: {maxpool_darknet_forward_32(layer, src, dest); break;}
    default: { printf("Unknown layer\n"); break; }
    }
}
...
void layer_forward_cpu(struct layer* layer, void* src, void* dest, void* workspace)
{
  switch (layer->prec)
    {
      case SINGLE: {layer_forward_32_cpu(layer, src, dest, workspace); break;}
    }
}
